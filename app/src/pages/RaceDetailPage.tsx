...
const RaceDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.warn("API MyFITri non disponibile.");
      }
  ...
  const handleAddToSeason = async () => {
    ...
    try {
      ...
      const isAdmin = authSession.user.email === ADMIN_EMAIL || latestProfile?.is_team_admin;
      
      console.log("🔍 DEBUG ADD RACE:", { 
          email: authSession.user.email, 
          isAdmin, 
          teamId: latestProfile?.team_id,
          profileName: latestProfile?.full_name 
      });

      if (isAdmin && latestProfile?.team_id) {
          console.log("📝 Inviando log ADMIN_ADD_RACE...");
          await logAdminAction(
              authSession.user.id, 
              latestProfile.team_id, 
              'ADMIN_ADD_RACE', 
              { race_title: race?.title, admin_name: latestProfile?.full_name }
          );
      }

      if (latestProfile?.full_name) {
        setParticipants(prev => [...prev, latestProfile.full_name]);
      }
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: `Guarda i dettagli per ${race?.title} su Race Planner!`,
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      {/* BANNER DI DEBUG - SE NON LO VEDI LA CACHE È ATTIVA */}
      <div className="bg-yellow-400 text-black p-4 rounded-2xl mb-8 font-black text-center animate-pulse border-4 border-black">
        ⚠️ MODALITÀ TEST ATTIVA (v6.2.5) - SE VEDI QUESTO IL CODICE È AGGIORNATO
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {apiData && (
            <div className="bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden">
               <div className="relative z-10">
                  <h3 className="text-xl font-black uppercase mb-8 flex items-center gap-3"><Info className="w-6 h-6 text-blue-400" /> MyFITri Info</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                    <div className="space-y-6">
                      <div className="flex items-center gap-4"><Clock className="w-5 h-5 text-slate-400" /><div><span className="text-[10px] font-black text-slate-500 uppercase block">Partenza</span><p className="font-black text-lg">{apiData.ora_partenza || "--:--"}</p></div></div>
                    ...
                    <div className="bg-white/5 p-6 rounded-[2.5rem] border border-white/10"><h4 className="text-[10px] font-black text-slate-400 uppercase mb-4">Percorsi</h4><p className="text-sm font-medium leading-relaxed opacity-80">{apiData.descrizione_percorsi || "Vedi regolamento."}</p></div>
                  </div>
               </div>
            </div>
          )}
        ...
        <div className="space-y-8">
           ...
           <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
             ...
             <button 
                ...
                className={`w-full mt-8 py-4 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg transition-all ${isRegistered ? 'bg-emerald-500 opacity-50 cursor-not-allowed' : 'hover:brightness-110'}`} 
                style={{ backgroundColor: !isRegistered ? (team?.primary_color || '#2563eb') : undefined }}
             >{isRegistered ? 'Gara in calendario' : 'Aggiungi alla mia stagione'}</button>
           </div>
