            ...
            {activeTab === 'atleti' && (
                <div className="space-y-4">
                    <div className="relative group max-w-md">
                        ...
                        <input type="text" placeholder="Cerca atleta per nome..." className="w-full pl-12 pr-4 py-3.5 bg-white border-2 border-slate-100 rounded-2xl focus:border-blue-500 outline-none text-sm font-medium shadow-sm transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {filteredProfiles.map((atleta) => (
                                        <tr key={atleta.id} className="hover:bg-slate-50/30 transition-colors">
                                            ...
                                            <td className="px-8 py-5"><select value={atleta.team_id || ''} onChange={(e) => handleUpdateAthleteTeam(atleta.id, e.target.value || null)} className="bg-slate-100 border-none rounded-xl px-3 py-2 text-xs font-black uppercase text-slate-600 outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"><option value="">Nessun Team</option>{teams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></td>
                                            ...
                                            <td className="px-8 py-5 text-right"><button onClick={() => handleDeleteAthlete(atleta.id, atleta.full_name)} className="p-2 text-slate-400 hover:text-red-600 transition-colors" title="Elimina atleta"><Trash2 className="w-4 h-4" /></button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        {teams.map((team) => (
                            <div key={team.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col relative overflow-hidden group">
                                ...
                                <div className="space-y-3 mb-8 relative z-10">
                                    <div className="flex items-center justify-between text-xs"><span className="font-bold text-slate-400 uppercase">Brand Colors</span><div className="flex gap-1"><div className="w-6 h-6 rounded-lg shadow-inner border border-white" style={{ backgroundColor: team.primary_color }}></div><div className="w-6 h-6 rounded-lg shadow-inner border border-white" style={{ backgroundColor: team.secondary_color }}></div></div></div>
                                    <div className="flex items-center justify-between text-xs"><span className="font-bold text-slate-400 uppercase">Atleti Iscritti</span><span className="font-black text-slate-700">{profiles.filter(p => p.team_id === team.id).length}</span></div>
                                </div>
            ...
            {isTeamModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white rounded-[3rem] p-10 max-w-lg w-full shadow-2xl animate-in zoom-in-95 overflow-y-auto max-h-[90vh]">
                        ...
                        <p className="text-slate-500 font-bold text-sm mb-8">Dettagli del team per il branding dinamico.</p>
                        <form onSubmit={handleSaveTeam} className="space-y-5">
                            <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Nome Squadra</label><input type="text" className="w-full px-5 py-3.5 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-blue-500 outline-none text-sm font-bold" value={teamForm.name} onChange={e => setTeamForm({...teamForm, name: e.target.value})} placeholder="Valdigne Triathlon" required /></div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Codice Iscrizione</label><input type="text" className="w-full px-5 py-3.5 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-blue-500 outline-none text-sm font-bold uppercase" value={teamForm.join_code} onChange={e => setTeamForm({...teamForm, join_code: e.target.value})} placeholder="VALDIGNE2026" required /></div>
                                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Colori Brand</label><div className="flex items-center gap-2"><input type="color" className="w-10 h-10 rounded-xl cursor-pointer" value={teamForm.primary_color} onChange={e => setTeamForm({...teamForm, primary_color: e.target.value})} /><input type="color" className="w-10 h-10 rounded-xl cursor-pointer" value={teamForm.secondary_color} onChange={e => setTeamForm({...teamForm, secondary_color: e.target.value})} /></div></div>
                            </div>
                            <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Logo Team</label><div className="flex items-center gap-4"><div className="w-16 h-16 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden shrink-0">{teamForm.logo_url ? <img src={teamForm.logo_url} alt="Preview" className="max-w-full max-h-full object-contain" /> : <Upload className="w-6 h-6 text-slate-300" />}</div><div className="flex-1"><input type="file" accept="image/*" id="logo-upload" className="hidden" onChange={handleUploadLogo} disabled={uploading} /><label htmlFor="logo-upload" className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest cursor-pointer transition-all ${uploading ? 'bg-slate-100 text-slate-400' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}>{uploading ? 'Caricamento...' : 'Carica Immagine'}</label></div></div><input type="text" className="w-full mt-3 px-5 py-3 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-blue-500 outline-none text-[10px] font-medium" value={teamForm.logo_url} onChange={e => setTeamForm({...teamForm, logo_url: e.target.value})} placeholder="O incolla URL diretto..." /></div>
                            <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Sito Web</label><input type="text" className="w-full px-5 py-3.5 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-blue-500 outline-none text-sm font-medium" value={teamForm.website_url} onChange={e => setTeamForm({...teamForm, website_url: e.target.value})} placeholder="https://..." /></div>
                            <button type="submit" className="w-full mt-6 py-4 bg-slate-900 text-white rounded-[1.5rem] text-xs font-black uppercase tracking-widest shadow-xl hover:bg-slate-800 transition-all flex items-center justify-center gap-2"><Save className="w-4 h-4" /> Salva Configurazione</button>
                        </form>
                    </div>
                </div>
            )}
